    ...
    def summarize(
        ...
        with st.spinner("Summarize Content ..."):
            ...
            if chain_type == "map_reduce":
                map_prompt_template = self.prompts_loader.web_summarize_map_templete()
                combine_prompt_template = self.prompts_loader.web_summarize_combine_templete(summarize_length)
                ...
                prompt_template = self.prompts_loader.web_summarize_refine_templete(summarize_length)
                PROMPT = PromptTemplate(template=prompt_template, input_variables=["text"])
                chain = load_summarize_chain(
                    llm, chain_type=chain_type, verbose=True, refine_prompt=PROMPT
                )
                response = chain(
